...
def create_csv_files():
    ...
    test_total_students = int(totals[5].get('test_num_students'))
    ...
    test_student_columns = ['student_id', 'initial_infection', 'c1']
    for test_student_id in range(0, test_total_students):
        # course_list = random.sample({0, 1, 2, -1}, 3)
        # course_list = heapq.nlargest(3, {0, 1, 2, -1}, key=lambda l: random.random())
        c1 = 0
        # c2 = course_list[1]
        ...
        if int(test_student_id) in range(0, 30):
            infected = 1
        else:
            infected = 0

        test_student_info_rows = {'student_id': test_student_id, 'initial_infection': infected, 'c1': c1,
                             }
        test_student_info.append(test_student_info_rows)
    ...
    try:
        ...
        print("I/O error")
    test_student_csv_file = "../input_files/test_student_info.csv"
    try:
        with open(test_student_csv_file, 'w') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=test_student_columns)
            writer.writeheader()
            for data in test_student_info:
                writer.writerow(data)
    except IOError:
        print("I/O error")

    teacher_csv_file = "../input_files/test_teacher_info.csv"
    try:
    ...
    course_csv_file = "../input_files/test_course_info.csv"
    try:
    ...
    classroom_csv_file = "../input_files/test_classroom_info.csv"
    try:
    ...
    community_csv_file = "../input_files/test_community_info.csv"
    try:
