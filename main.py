...
def run_training(env, shared_config_path, alpha, agent_type, is_sweep=False):
    ...
    print("Is main thread in run_training:", threading.current_thread() == threading.main_thread())
...
def run_sweep(env, shared_config_path, agent_type):
    shared_config = load_config(shared_config_path)
    print("Is main thread in run_sweep:", threading.current_thread() == threading.main_thread())
...
def main():
    ...
    print("Is main thread before mode selection:", threading.current_thread() == threading.main_thread())
    ...
    if args.mode == 'train':
        ...
        # Make sure wandb.agent is called in the main thread
        if threading.current_thread() == threading.main_thread():
            wandb.agent(sweep_id, function=lambda: run_sweep(env, shared_config_path, args.agent_type))
        else:
            raise RuntimeError("wandb.agent() must be called in the main thread")
    else:
