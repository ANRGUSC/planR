...
def get_avg_alpha(alpha_p):
    ...
    training_allowed_df = pd.read_json(
        f'results/E-greedy/{training_name_greedy}-{episodes}-{alpha}episode_allowed.json')
    training_infected_df = pd.read_json(
        f'results/E-greedy/{training_name_greedy}-{episodes}-{alpha}episode_infected.json')
