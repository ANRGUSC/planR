...
#                 max_rew = -np.inf
#                 best_action = -1
#                 for action in range(self.env.action_space.nvec[0]):
#                     rew = self.test_env.campus_state.get_reward_of_action_without_update([action], alpha)
...
#                 scaled_best_actions = best_action * (100. / (self.num_actions-1))
#                 print(scaled_best_actions)
#                 best_action_alpha_list = [scaled_best_actions, alpha]
#                 test_next_obs, test_rewards, test_terminations, test_truncations, test_infos = self.test_env.step(best_action_alpha_list)
#                 test_next_obs_rb = np.array(test_next_obs, dtype=float)
#                 best_actions_rb = np.array(best_action, dtype=float)
...
#                 actions_rb = np.array(actions, dtype=float)
#                 rb.add(obs_rb, next_obs_rb, actions_rb, rewards, terminations, infos)
#                 rb_test.add(obs_rb, test_next_obs_rb, best_actions_rb, test_rewards, test_terminations, test_infos)
#                 episode_tot_reward += rewards
#                 episode_len += 1
#                 done = terminations or truncations
#                 i += 1
...
#                 if global_step % self.train_frequency == 0:
#                     data = rb.sample(self.batch_size)
#                     target_data = rb_test.sample(self.batch_size)
#                     with torch.no_grad():
#                         target_max, _ = target_network(data.next_observations.to(dtype=torch.float32)).max(dim=1)
#                         td_target = abs(target_data.rewards.flatten()-data.rewards.flatten()) + self.gamma * target_max * (1 - data.dones.flatten())
#                     observations = data.observations.float()
#                     old_val = q_network(observations).gather(1, data.actions).squeeze()
#                     # old_val = np.array([q_network(observations).item()])
#                     loss = F.mse_loss(td_target, old_val)
#                     # loss = F.mse_loss(data.rewards.flatten(), target_data.rewards.flatten())
#                     wandb.log({
#                         f'sweep {self.timestamp_day}/loss': loss,
#                     })
#                     # optimize the model
